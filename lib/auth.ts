...
export const authOptions: NextAuthOptions = {
  providers: [
    GoogleProvider({
      ...
      // returns. We don't need that: the `userinfo` endpoint below (a plain
      // `fetch` call) already gets us the user's verified profile straight
      // from Google over HTTPS, which is exactly what `profile()` maps
      // below. Turning it off skips the id_token check.
      idToken: false,
      // openid-client's Issuer object needs its own `issuer` value set even
      // for a plain (non-OIDC) OAuth callback — without it, `oauthCallback`
      // throws "issuer must be configured on the issuer" the moment Google
      // redirects back, because it validates the `iss` query parameter
      // Google now appends to the callback (RFC 9207) against this value.
      // Normally `issuer` comes from the `wellKnown` discovery document we
      // skipped above, so it has to be supplied directly instead.
