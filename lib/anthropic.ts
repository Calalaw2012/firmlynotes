...
const EVENT_ITEM_SCHEMA = {
  ...
  properties: {
    ...
    attendees: {
      type: "array",
      description:
        "People to actually invite to this event -- extracted ONLY from phrases that clearly show participation or responsibility, like 'with Sarah', 'invite the Hendricks', 'cc opposing counsel', 'meet with John', 'Monica and Peter to attend', 'attendees: Monica, Peter', 'X and Y will be there', 'loop in Tom', 'bring Sam', 'Tom joining', 'join us: X, Y', 'X on the call', 'need Jane there', a shorthand 'w/' used the same way as 'with', OR -- especially for a filing/response deadline, which has no in-person meeting to 'attend' -- an assignment/responsibility phrase like 'X will prepare/draft/handle/respond to it', 'assigned to X', 'X is handling this', 'have X take care of it'. A person's name showing up elsewhere in the note -- as part of a case/matter name, party name, or subject line (e.g. 'Smith v. Jones', 'the Dillon matter', 're: Johnson deposition') -- is NOT by itself a reason to add them as an attendee. Only add someone if the note separately indicates they are attending, invited, cc'd, or responsible for it. Do not include the note-taker themselves.",
      items: {
        type: "object",
        properties: {
          name: {
    ...
    serviceDate: {
      type: ["string", "null"],
      description:
        "Only set when courtDetected is non-null. The date this event's clock started -- service, filing, or receipt of the triggering document (including a discovery request under interrogatories/production/admissions) -- resolved to YYYY-MM-DD, ONLY if the note states one. Null if the note gives no such date; never guess or default it to the note's own date or today.",
    },
    ...
    documentServed: {
      type: "string",
      description:
        "Only fill in when courtDetected is non-null AND the note says what was served, filed, or received. Reworded into a short, calendar-ready noun phrase, capitalized like a document title (e.g. 'served with a motion to dismiss' -> 'Motion to Dismiss'; 'they filed for summary judgment' -> 'Motion for Summary Judgment'; 'interrogatories received' -> 'Interrogatories'; 'they sent a request for production' -> 'Request for Production'; 'served requests for admission' -> 'Requests for Admission'). Empty string if the note doesn't say what document triggered the deadline -- never guess.",
    },
    caseNumber: {
      type: "string",
      description:
...
- Attendees: pull out people the note says to meet with, invite, cc, have attend, or have handle/prepare/respond to that specific event -- including phrasing like "X to attend", "attendees: X, Y", "X and Y will be there", "loop in X", "bring X", "X joining", "join us: X, Y", "X on the call", "need X there", a shorthand "w/" used the same way as "with", or -- not just "with X" -- an assignment/responsibility phrase like "X will prepare/draft/handle/respond to it", "assigned to X", "X is handling this", "have X take care of it". A filing/response deadline has no in-person meeting to "attend", so responsibility IS the participation cue there: whoever the note names as preparing, drafting, handling, or responding to it is an attendee, so the deadline shows up on their calendar too. A name is only an attendee if the note says that person is participating, invited, cc'd, attending, joining, or responsible for it -- a name that appears merely as part of a case/matter name, party name, or subject reference (e.g. "Smith v. Jones", "the Dillon matter", "re: Johnson deposition", a case caption, a docket title) is NOT an attendee unless the note separately says that person is attending, should be invited, or is handling it. When in doubt between two people-sounding readings, prefer extracting the one that plausibly indicates participation over silently dropping it, but never invent an attendee the note gives no participation/responsibility cue for at all. Only fill in an email if the note literally contains one; otherwise leave email as "" and put their name in "name" exactly as written, using the fullest form the note gives (e.g. "Sarah", "Sarah Chen", "the Hendricks", "opposing counsel on Mercer") — a name-only attendee gets matched against the user's contacts (and any name the user has previously resolved to an email) afterward, so don't guess or fabricate an address.
